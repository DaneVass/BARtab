...
process CUTADAPT_READS{
  ...
  if(params.mode == "single-cell") {
    ...
    cutadapt -j ${params.threads} -a ${params.downconstant} --trimmed-only --max-n=0 -m ${params.min_readlength} -e ${params.constantmismatches} ${reads} > ${sample_id}.trimmed_3.fastq 2>> ${sample_id}.cutadapt.log
